...
def init_db() -> None:
    with _db_connect() as conn:
        ...
        # Same PRAGMA-guarded ALTER pattern again — /panel used to be gated
        # purely by MONITOR_PANEL_TOKEN (see below), a mechanism that
        # AUTH_ENABLED=true now ignores completely (see _panel_admin_guard).
        # Caught live on dev before this ever reached prod: a plain ALTER
        # leaves every existing row at is_admin=0 (the column default), and
        # with the token path closed that means /panel becomes unreachable
        # for EVERYONE on an upgrading install, including whoever already
        # holds the token — not a graceful "ask an existing admin to grant
        # you", but a hard lockout with no path back in short of editing the
        # DB by hand. So this promotes the earliest-created existing account
        # (by created_at, same tie-break logic as "first ever" for a brand
        # new install in _get_or_create_user) the moment the column first
        # appears — mirrors that bootstrap rule instead of introducing a
        # second one.
        if "is_admin" not in existing_user_cols:
            conn.execute("ALTER TABLE users ADD COLUMN is_admin INTEGER NOT NULL DEFAULT 0")
            _earliest_user = conn.execute(
                "SELECT id FROM users ORDER BY created_at ASC, id ASC LIMIT 1"
            ).fetchone()
            if _earliest_user is not None:
                conn.execute(
                    "UPDATE users SET is_admin = 1 WHERE id = ?", (_earliest_user[0],)
                )
...
def _get_or_create_user(email: str) -> int:
    ...
    #
    # The very first account ever created gets is_admin=1 for free — same
    # chicken-and-egg problem as can_invite's bootstrap invite above, solved
    # the same way but one step earlier: without this, nobody could ever
    # reach /panel to grant is_admin to anyone, including themselves. The
    # "is this the first row" check and the insert share one connection/
    # transaction, so there's no window for a second concurrent registration
    # to also read "zero users" — moot at this project's beta scale, but
    # cheap to get right anyway.
    now = datetime.now(timezone.utc).isoformat()
    with _db_connect() as conn:
        is_first_user = conn.execute("SELECT 1 FROM users LIMIT 1").fetchone() is None
        conn.execute(
            "INSERT INTO users (email, created_at, tos_version, tos_accepted_at, is_admin) "
            "VALUES (?, ?, ?, ?, ?) ON CONFLICT(email) DO NOTHING",
            (email, now, TOS_VERSION, now, int(is_first_user)),
        )
        return conn.execute(
...
def _get_user_by_session_cookie(raw: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    with _db_connect() as conn:
        ...
        row = conn.execute(
            """
            SELECT u.id, u.email, u.can_invite, u.is_admin, ls.expires_at FROM login_sessions ls
...
async def me(request: Request, response: Response):
    if not AUTH_ENABLED:
        return {"logged_in": False, "can_invite": False, "is_admin": False}
    raw_cookie = request.cookies.get(LOGIN_COOKIE_NAME)
    user = await run_in_threadpool(_get_user_by_session_cookie, raw_cookie)
    if user is None:
        return {"logged_in": False, "can_invite": False, "is_admin": False}
    ...
    with _db_connect() as conn:
        ...
        users = [
            dict(row) for row in conn.execute(
                """
                SELECT u.id, u.email, u.created_at, u.can_invite, u.is_admin,
...
def _render_panel_html(token: str, auth_mode: bool) -> str:
    # auth_mode picks how the page's own POST forms/links authenticate
    # themselves back to the server: under AUTH_ENABLED the browser's login
    # cookie already does that (see _panel_admin_guard), so URLs here stay
    # clean; the legacy MONITOR_PANEL_TOKEN install (AUTH_ENABLED=false)
    # still needs the token carried in every link, same as before this
    # change.
...
"""
    ...
    def _panel_form_action(path: str, user_id: int) -> str:
        if auth_mode:
            return f"{path}?user_id={_esc(user_id)}"
        return f"{path}?token={_esc(token)}&user_id={_esc(user_id)}"
    ...
    if stats["users"]:
        ...
        for u in stats["users"]:
            ...
            invite_action = "Revoke" if u["can_invite"] else "Grant"
            invite_badge = "✅ can invite" if u["can_invite"] else "—"
            admin_action = "Revoke admin" if u["is_admin"] else "Make admin"
            ...
            user_rows.append(
                ...
                f'<span class="user-badge">{invite_badge}</span>'
                f'<span class="user-badge">{admin_badge}</span>'
                f'<form method="post" action="{_panel_form_action("/panel/toggle-invite", u["id"])}" class="user-form">'
                f'<button type="submit">{invite_action}</button>'
                f"</form>"
...
<a class="download" href="{download_href}">Download statistics</a>
</div>
</body>
</html>"""


# Shared gate for /panel and its POST actions. Two independent modes, never
# both active for the same request:
#   - AUTH_ENABLED=true: real login cookie + users.is_admin. No token in the
#     URL at all — a pentest flagged the old query-string token as exactly
#     the kind of thing that leaks into logs/referrers/history, and the
#     login system already gives us a strictly better mechanism for free.
#   - AUTH_ENABLED=false (plain self-host, no RESEND_API_KEY): the original
#     MONITOR_PANEL_TOKEN comparison, untouched — there's no login system to
#     gate on in that mode.
# Always raises 404, never 401/403, either way — a random visitor shouldn't
# be able to tell "wrong token"/"not an admin" apart from "route doesn't
# exist", same reasoning as every other silent-404 gate in this file.
#
# Returns the raw session cookie value when AUTH_ENABLED (so the caller can
# renew it on its own outgoing response — see _set_session_cookie), None
# otherwise. Deliberately does NOT take/mutate an injected `response`: a
# route here builds and returns its own Response/RedirectResponse instance
# rather than a plain dict, and FastAPI only auto-merges the injected
# response's headers into a dict/model return value, not into one you
# construct and return yourself (same reason session_takeover() below sets
# the cookie directly on the Response object it's about to return, not on a
# separate injected one).
async def _panel_admin_guard(request: Request, token: str) -> Optional[str]:
    if AUTH_ENABLED:
        raw_cookie = request.cookies.get(LOGIN_COOKIE_NAME)
        user = await run_in_threadpool(_get_user_by_session_cookie, raw_cookie)
...
def _panel_redirect_url(token: str) -> str:
    return "/panel" if AUTH_ENABLED else f"/panel?token={token}"
...
async def monitor_panel(request: Request, token: str = "", download: bool = False):
    ...
    html_body = await run_in_threadpool(_render_panel_html, token, AUTH_ENABLED)
    panel_response = HTMLResponse(html_body)
    if download:
        filename = f"hvac-guide-{GIT_COMMIT}-{datetime.now(timezone.utc).strftime('%d-%m-%Y')}.html"
        panel_response.headers["Content-Disposition"] = f'attachment; filename="{filename}"'
    if AUTH_ENABLED:
        _set_session_cookie(panel_response, raw_cookie)
...
def _set_is_admin(user_id: int, is_admin: bool) -> None:
    with _db_connect() as conn:
...
@app.post("/panel/toggle-invite", include_in_schema=False)
async def toggle_invite(request: Request, token: str = "", user_id: int = 0):
    raw_cookie = await _panel_admin_guard(request, token)
    with _db_connect() as conn:
        row = conn.execute("SELECT can_invite FROM users WHERE id = ?", (user_id,)).fetchone()
    if row is None:
...
# Same pattern as toggle-invite, plus one guard toggle-invite doesn't need:
# refuse to revoke the last remaining admin, so a solo admin can't lock
# themselves out of /panel by clicking the wrong row.
@app.post("/panel/toggle-admin", include_in_schema=False)
async def toggle_admin(request: Request, token: str = "", user_id: int = 0):
    raw_cookie = await _panel_admin_guard(request, token)
    with _db_connect() as conn:
    ...
    if row[0] and await run_in_threadpool(_admin_count) <= 1:
        raise HTTPException(status_code=400, detail="cannot revoke the last remaining admin")
    await run_in_threadpool(_set_is_admin, user_id, not row[0])
    redirect = RedirectResponse(url=_panel_redirect_url(token), status_code=303)
    if AUTH_ENABLED:
        _set_session_cookie(redirect, raw_cookie)
    return redirect
