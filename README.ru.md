...
└── app/
    ...
    └── static/
        ...
        ├── graph.json             # сам граф решений — НЕ поставляется в этом публичном репо
        │                          # (см. "Как редактировать граф вопросов" ниже), но именно сюда
        │                          # его кладёт приложение и правит без пересборки контейнера
...
**Этот репозиторий сейчас не поставляет `graph.json`.** Полный
...
репо целиком. Урезанный demo-граф (пара типов оборудования, только базовый
уровень) планируется как замена для self-host-юзеров, но пока не собран —
до этого self-host этого репо "как есть" означает написать свой
`app/static/graph.json` с нуля, по формату узла ниже.
