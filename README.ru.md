...
напрямую — это весь воркфлоу, следующий абзац на него не влияет.

Собственная копия `graph.json` в этом репозитории дополнительно генерируется
из `app/graph_src/` (`graph-structure.json` + `content/<lang>.json`, плюс
`tools/build_graph.py`/`tools/split_graph_content.py`/
`tools/validate_content.py`) — внутреннее удобство для мейнтейнеров, чтобы
держать RU/EN/будущие языки отдельно от структуры графа. Это опциональная
инфраструктура именно для того, как контент редактируют мейнтейнеры этого
репо — если форкаете проект, ничего из этого не нужно, просто продолжайте
редактировать `graph.json` напрямую, как описано выше.
