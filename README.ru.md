...
Интерактивный чек-лист-траблшутер для HVAC/R оборудования (RTU, сплит, печи,
тепловые насосы, чиллеры, холодильное оборудование, VRF/VRV). Вопросы идут по графу решений в
...
**Кто это писал.** Диагностический контент — граф вопросов, терминология,
понимание того, что является разумным следующим шагом, когда руки уже на
оборудовании, — от автора, HVAC/R-энтузиаста из Виннипега. Большая часть
кода вокруг написана с Claude Code, ИИ-ассистентом для программирования, по
этому направлению; в коммитах, где это применимо, стоит трейлер
`Co-Authored-By`. Диагностический контент проверяется на соответствие
полевой практике до выкладки: сочинять советы по HVAC ИИ не позволено, а
когда он пытался — это баг, который ревью и должно ловить.
...
прода без сопутствующих объяснений; [SECRETS.md](SECRETS.md) —
опционально: как убрать API-ключ из `.env` совсем, через Bitwarden
Secrets Manager (работает и на бесплатном тарифе; сам документ —
на английском, как и остальная repo-документация);
[CONTRIBUTING.md](CONTRIBUTING.md) — чем помочь полезнее всего (правки по
полю ценнее рефакторингов); [SECURITY.md](SECURITY.md) — как приватно
сообщить об уязвимости; [LICENSE](LICENSE) — AGPL-3.0;
[CHANGELOG.md](CHANGELOG.md) —
история изменений по датам (ведётся только на русском — единственный
реальный читатель — автор).

## Лицензия

[AGPL-3.0](LICENSE). Пользуйтесь, меняйте, разворачивайте — но если вы
поднимаете изменённую версию как сетевой сервис, у тех, кто ей пользуется,
есть право получить эти изменения в исходниках. Выбрана вместо
разрешительной лицензии именно поэтому.

## Дисклеймер

Это независимый личный проект, который я делаю и поддерживаю в свободное
время — не аффилирован ни с одним производителем HVAC/R, поставщиком
хладагента или органом стандартизации. Предоставляется как есть, без каких-
либо гарантий, и я не несу ответственности за последствия использования
инструмента. Он не заменяет документацию производителя, местные нормы или
суждение квалифицированного, лицензированного специалиста. (Это отдельно от
более узкого AI-дисклеймера, который показан баннером в приложении и
добавляется к каждому ответу AI — тот касается конкретно ответов AI, этот —
использования инструмента в целом.)

**Безопасность.** Диагностика работающего оборудования — это цепи под
напряжением, хладагент под давлением, а на отопительном оборудовании ещё и
горение с угарным газом. Ничто здесь не отменяет LOTO, местные нормы и вашу
собственную подготовку. Если шаг чек-листа небезопасен на конкретной машине
перед вами — не делайте его.

**Область применения.** Контент рассчитан на обычное коммерческое и
лёгкое коммерческое холодоснабжение и кондиционирование: RTU, сплит-системы,
VRF/VRV, чиллеры, холодильное оборудование walk-in/reach-in и печи с
принудительной подачей воздуха. Он **не** покрывает низкотемпературное
холодоснабжение (примерно ниже -30 °C), у которого свои режимы отказов и
свои правила безопасности, и **не** покрывает оборудование, отапливаемое
керосином, дизелем, мазутом, углём, дровами, пеллетами или другим твёрдым/
жидким/биотопливом — контент по отопительному оборудованию здесь
рассчитан на газ или электричество. Лучше всего выверен контент по RTU; остальные
классы оборудования подтягиваются к тому же уровню, поэтому к их советам
стоит относиться с дополнительным скепсисом и проверять, существует ли
упомянутый компонент на вашей машине вообще.

**Терминология и единицы** следуют североамериканской практике. Нормы,
требования по рекуперации и лицензированию различаются по юрисдикциям —
действует ваша.
...
```
hvac-guide/
├── docker-compose.yml       # локально/dev: без Caddy, без TLS/auth, порт 8080
├── docker-compose.prod.yml # прод: добавляет сервис Caddy (reverse proxy) + volume для SQLite
├── Caddyfile                 # reverse proxy: basic auth (Phase 1), upstream через CADDY_UPSTREAM
├── Dockerfile
├── requirements.txt
├── .env.example
├── .githooks/                # post-commit/post-checkout/post-merge — держат GIT_COMMIT в .env актуальным
├── tools/
│   └── visualize_graph.py  # только для разработки: рендерит graph.json в Mermaid, не попадает в образ
├── README.md / README.ru.md, ROADMAP.md, DEPLOY.md, commands.md, CHANGELOG.md
└── app/
    ├── main.py            # FastAPI: /api/ai-assist (proxy к Anthropic API),
    │                      # /api/log-session (история чек-листов), /api/version + отдача статики
    ├── graph_demo_src/    # источник для static/graph.demo.json ниже (структура + RU/EN текст,
    │                      # тот же формат split, что у приватного graph_src/ мейнтейнера — см.
    │                      # tools/build_demo_graph.py), закоммичен здесь, не в .gitignore
    └── static/
        ├── index.html
        ├── style.css
        ├── app.js                 # логика графа, конвертация единиц, вызовы AI, расчёт P-T калькулятора
        ├── graph.json             # сам граф решений — не трекается в этом репо (см. "Как
        │                          # редактировать граф вопросов" ниже), но именно сюда его
        │                          # кладёт приложение и правит без пересборки контейнера
        ├── graph.demo.json        # урезанный demo-граф (RTU + Split + Chiller, только базовый
        │                          # уровень), трекается — Dockerfile копирует его в graph.json
        │                          # при сборке образа, если своего graph.json ещё нет на диске
        ├── manufacturers.json     # список производителей для шага после выбора типа оборудования
        ├── refrigerants.json      # манифест хладагентов для P-T калькулятора (id/name/file)
        └── refrigerants/*.json    # P-T таблицы по каждому хладагенту, отдельный файл на хладагент
```

Один контейнер, порт 8000 внутри / 8080 снаружи в dev (`docker-compose.yml`);
за Caddy в проде (`docker-compose.prod.yml`, см. [DEPLOY.md](DEPLOY.md)).
...
просто пересобрать контейнер после изменений). Просто редактируйте его
напрямую — это весь воркфлоу.

**Сам `graph.json` больше не трекается в этом репозитории.** Полный
диагностический граф (все типы оборудования, все result-узлы) раньше был
обычным закоммиченным файлом здесь — а значит, любой клон репозитория
отдавал весь накопленный полевой контент бесплатно. Теперь этот контент
живёт в приватном репозитории, и деплой-пайплайн мейнтейнера доставляет
собранный `graph.json` прямо на рабочий сервер, минуя git-историю этого
репо целиком.

**Self-host при этом по-прежнему просто работает, только с меньшим
графом.** `app/static/graph.demo.json` — RTU, Split и Chiller, только
базовый уровень — трекается здесь, и Dockerfile копирует его в
`graph.json` при сборке образа, если такого файла ещё нет на диске (т.е.
на любом свежем клоне). `docker compose up` остаётся ровно тем же
self-host в 3 команды из Quick start — просто стартуете с урезанного
demo-контента вместо полного графа, и растите его дальше как обычно,
редактируя `app/static/graph.json` напрямую. (`app/graph_demo_src/` и
`tools/build_demo_graph.py` — это то, чем мейнтейнер *этого* репо
пересобирает demo-подмножество из приватного полного графа, форку/
self-host'у это не нужно запускать самостоятельно, т.к. зависит от
контента, которого у вас нет — просто редактируйте обычный JSON.)
...
- `result` — финальная рекомендация. Поля `severity` (`info` / `warning` /
  ...
  Опциональный массив `checklist` добавляет интерактивный чек-лист
  выполненных действий под рекомендацией — см. "Персистентность чек-листа"
  ниже.
...
цепочке считывания давлений/температур для расчёта перегрева/переохлаждения.
...
**Текст системного промпта выше — публичный дефолт, не обязательно то,
что реально шлёт прод этого же проекта.** Та же логика, что у
`graph.json`/`graph.demo.json` выше: приватный репо мейнтейнера держит
более полную, обкатанную в поле версию диагностических инструкций и
промптов model/nameplate-lookup, доставляется на работающий контейнер как
`app/prompts_private.py` (в `.gitignore`, отсутствует на свежем клоне).
Если файл есть — `main.py` подгружает его при старте и подставляет этот
текст; если нет — используются промпты, определённые прямо в `main.py`,
ровно как показано здесь, без ошибок и без потери функциональности —
просто менее детальные подсказки по нескольким конкретным нюансам. Текст,
критичный для безопасности (юридический дисклеймер, строка отказа при
попытке инъекции, редирект по CSAM-подозрению), в этот оверрайд никогда
не входит — это всегда фиксированная строка, которую определяет `main.py`,
независимо от того, какой вариант промпта активен. Self-host'ер, который
хочет свой приватный слой промптов, может написать Python-модуль с теми
же тремя функциями, которые ждёт загрузчик в `main.py` (см.
`_load_private_prompt_overrides()`), и положить его по этому же пути.
...
## Персистентность чек-листа (продолжение после перезагрузки страницы)

Отдельно от "Истории чек-листов" выше — там односторонний аналитический лог
пройденных/завершённых чек-листов. Здесь — живое, восстанавливаемое
состояние незавершённого прохождения, хранится в той же `data/sessions.db`,
но в отдельной таблице `sessions`, чтобы техник мог продолжить ровно с того
места, где остановился, после закрытия вкладки, потери связи или
перезагрузки браузера.

**graph.json:** опциональное поле `checklist` на `result`-узле — простой
список пунктов "что сделать после рекомендации", показывается прямо под
...
"nc_sh_high": {
  ...
  "severity": "warning",
  "checklist": [
    {"id": "airflow_checked", "type": "checkbox", "label": {"ru": "Проверен airflow на испарителе", "en": "..."}},
    {"id": "subcooling", "type": "field", "unit": "Δ°F", "label": {"ru": "Замеренный subcooling", "en": "..."}}
  ]
}
```

- `type: "checkbox"` — простая галочка-подтверждение.
- `type: "field"` (опционально с `unit`, показывается как placeholder поля)
  — короткий текстовый/числовой ввод, например замер, сделанный при
  выполнении рекомендации.
- Живая строка прогресса "Выполнено: X из Y" над пунктами — чекбокс
  засчитывается по факту отметки, поле — по факту непустого значения.

**Состояние фронтенда → бэкенд:** полное состояние прохождения браузером
(`currentId`, `history`, `answers`, выбор производителя/хладагента) и
значения чекбоксов/полей чек-листа — два отдельных JSON-блоба, формат
которых задаёт сам фронтенд — бэкенд их просто хранит и возвращает как есть
(`SessionUpsertRequest.node_path` / `.checklist_state` в `main.py`,
ограничены по размеру сериализации, но не по форме). Сохраняются через
debounced `POST /api/session` (через 800мс после последнего изменения —
каждый ответ, каждая правка чек-листа) в таблицу `sessions`, с тем же
`session_id`, что уже используется для истории чек-листов выше.

**Продолжение:** браузер помнит свой `session_id` в `localStorage`. При
заходе, если сессия с этим id всё ещё `status: "active"` на бэкенде,
пользователь видит экран "Продолжить? / Начать заново?" перед рендером
графа. "Продолжить" восстанавливает точное сохранённое состояние (включая
прогресс чек-листа); "Начать заново" помечает старую сессию как `abandoned`
— не удаляет, просто больше не предлагает для восстановления — и начинает
по-настоящему новую.

**Завершение:** кнопка "Завершить чек-лист" (показывается только если у
текущего result-узла есть `checklist`) ведёт на экран-сводку — все вопросы
с ответами по текущему пути плюс тот же чек-лист, всё ещё редактируемый —
заканчивается действием "Завершить", которое ставит `status: "completed"` и
фиксирует `completed_at`. `completed`-сессии не затрагиваются TTL-очисткой
ниже — они хранятся бессрочно как будущие данные для outcome-tracking (см.
ROADMAP.md).

**Очистка заброшенных сессий:** `active`-сессия, которую не трогали дольше
`SESSION_ABANDON_TTL_HOURS` (3 часа, пока не вынесено в env) — помечается
`abandoned` фоновой `asyncio`-задачей в `main.py` (проверка каждые 30 минут,
без cron) — строки никогда не удаляются, просто больше не предлагаются для
восстановления.

**Защита от злоупотреблений:** `SESSION_RATE_LIMIT` (по умолчанию 20/мин с
IP, через `slowapi`) покрывает и `POST /api/session`, и
`GET /api/session/{id}` — отдельно от `LOG_SESSION_RATE_LIMIT`, так как
debounced-сохранение срабатывает значительно чаще, чем разовая запись в
историю чек-листов.
