...
"node_id": {
  ...
  "text": "Текст вопроса",
  "options": [
    {"label": "Вариант ответа", "next": "id_следующего_узла"}
  ]
}
```
...
терминологию, отвечает по-русски). Модель задаётся через `ANTHROPIC_MODEL`
(по умолчанию `claude-sonnet-5`), ключ — через `ANTHROPIC_API_KEY`.
