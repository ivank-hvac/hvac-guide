...
Проверить целостность графа (что все `next` — из `options`, `thresholds` и
`measurement` — ссылаются на существующие узлы) можно так:
...
- **Перед добавлением новой ссылки — проверяйте её вручную** (WebFetch/поиск),
  не полагайтесь на память модели. При составлении текущего списка
  оказалось, что: Delhi Industries давно поглощена Canarm и
  `delhi-industries.com` теперь редиректит на `canarm.com/hvac` (домен
  оставлен как более "постоянный" адрес); "ICE" как канадский
  производитель — это ICE Western (Calgary), а не что-то другое; часть
  сайта под тем же неймингом (`icesales.ca` / ICE Northern Sales) сейчас
  торгует только запчастями для устаревших юнитов, а не самим
  оборудованием, поэтому в списке именно `icewestern.com`.
